...
module.exports = new LocalStrategy({
	...
	}, function(req, u_id, password, done) {
        // 요청 파라미터 중 name 파라미터 확인
        var paramName = req.body.name || req.query.name;
	 
		console.log('passport의 local-signup 호출됨 : ' + u_id + ', ' + password + ', ' + paramName);
		
	    // findOne 메소드가 blocking되지 않도록 하고 싶은 경우, async 방식으로 변경
	    process.nextTick(function() {
	    	var database = req.app.get('database');
		    database.UserModel.findOne({ 'u_id' :  u_id }, function(err, user) {
		        // 에러 발생 시
		        if (err) {
		            return done(err);
		        }
		        ...
		        if (user) {
		        	...
		        	// 모델 인스턴스 객체 만들어 저장
		        	var user = new database.UserModel({'u_id':u_id, 'password':password, 'name':paramName});
